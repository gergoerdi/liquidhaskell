...
addNoInlinePragmasToBinds tcg = tcg{ tcg_binds = go False (tcg_binds tcg) }
  where
    ...
    markHsBind isAbs = \case
        VarBind ext var rhs -> VarBind ext (markId isAbs var) (go False rhs)
        FunBind ext var matches -> FunBind ext (markId isAbs <$> var) (go False matches)
        PatBind ext lhs mult rhs -> PatBind ext lhs mult (go False rhs)
        PatSynBind{} -> error "markNoInline: PatSynBind"
    ...
    markId isAbs var
      ...
      | otherwise
      = var `setInlinePragma` neverInlinePragma{ inl_inline = NoInline NoSourceText}
